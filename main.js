...
function checkInvadersReachedEdge(invader) {
    if (invader.x <= 10) {
        swapDirection = true
    }

    if (invader.x + invader.width >= canvas.width - 10) {
        swapDirection = true
    }
}

function removeInvaderFromColumn(invader) {
    const invaderColumn = invader.col;
    if (invadersInColumns[invaderColumn]) {
        const index = invadersInColumns[invaderColumn].indexOf(invader);
        if (index > -1) {
            invadersInColumns[invaderColumn].splice(index, 1);
        }
    }
}
...
function scheduleUFO() {
    if (!ufoOnScreen && !pauseGame) {
        ...
        setTimeout(() => {
            ...
            ufo = new UFO(ufoImage, ufoX, window.topMargin, 3.5, ufoDirection, ctx);
...
function update() {
    ...
    if (playerProjectile) {
        ...
        if (playerProjectile.y < window.topMargin) {
            const explosion = new ParticleExplosion(playerProjectile.x + (playerProjectile.width / 2.0), playerProjectile.y, 25, 25);
            explosions.push(explosion);
            playerProjectile = null;
            return
        } else {
            ...
            if (playerProjectile) {
                barriers.forEach((barrier, bIndex) => {
                    if (barrier.collisionDetection(playerProjectile)) {
                        ...
                        const explosion = new ParticleExplosion(playerProjectile.x + (playerProjectile.width / 2.0), playerProjectile.y, 100, 100);
                        explosions.push(explosion);
                        playerProjectile = null;
                    }
                });
